...
As the core saturates, as the permeability of the core decreases significanlty compared to the linear region, reluctance increases and thus the inductance decreases. This is why the inductance values are significantly lower than the linear case, especially at the aligned position where saturation is most severe. The stored energy also decreases due to the reduced inductance and the fact that the core cannot store as much magnetic energy when it is saturated. The results confirm that saturation has a significant impact on the machine performance, reducing both the inductance and the stored energy, which would in turn affect the torque production.
